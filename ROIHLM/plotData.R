...
for (i in 1:length(files)){
 ...
 ##
 # get the mean
 m<-data.frame(y=sapply(AgeC, function(el){ d[1,'FVINTRCP'] + el*d[1,'FVAGEC']}), Age=Age, sex=3,id='mean')
 # get individuals
 d<-do.call("rbind",lapply(1:nrow(d),function(i){data.frame(y=sapply(AgeC, function(el){ d[i,'ECINTRCP'] + el*d[i,'ECAGEC']}), Age=Age, sex=d[i,'SEXNUM'],id=d[i,1])}))
 # cobmine the two
 d<-rbind(d,m)
 # make sex a factor (attach mean to sex for displaying) and give names
 d$sex<-as.factor(d$sex)
 levels(d$sex)<-c('female','male','mean')

 # setup plot
 g <- ggplot(data=d,aes(x=Age,y=y,color=sex,size=sex))

 png(paste('img_R20121018/supfig3-',sub(' ','_',t),'.png',sep=''))
 print(
...
files <- paste('csv_parallel',c('dlPFC_L_invageC.csv' ,  'dlPFC_R_invageC.csv',  'vlPFC_L_invageC.csv'),sep='/');
# problem file, merged sex55 with another file
files <- sub('(vlPFC_L)','20121018_\\1',files)
...
for (i in 1:length(files)){
 ...
 if(length(d$SEX55)>0)   d$SEXNUM<-d$SEX55+.5
