...
ind_sort <- rev(sort(
                  sapply(
                          1:dim(pvalsTbl)[2],
                          function(i){
                                  ind <- which(pvals[,i]<0.05)
                                  if(length(ind)==0) 0 else max(ind)
                          }
...
par(mar=c(5,7,4,1), cex.axis=2, cex.lab=3, cex.main=3)
...
## color bar
image(y=exp(prange), z=t(as.matrix(prange)), col=col, xaxt="n", ylab="", main="p")
dev.off()
